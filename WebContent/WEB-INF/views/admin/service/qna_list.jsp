...
<%--   <!-- 검색 폼 -->
  <div class="pull-left">
    <form method="get" action="${pageContext.request.contextPath}/admin/service/qna_list_list.do" style="width: 200px">
      ...
      <c:choose>
        <c:when test="${fn:length(qnaList) > 0}">
          <c:forEach var="qnaList" items="${qnaList}">
              <tr>
                <td class="text-center">${qnaList.idx}</td>
                <td class="text-center">
                  <c:url var="readUrl" value="/admin/service/qna_read.do">
                    <c:param name="qna_no" value="${qnaList.idx}"/>
                  </c:url>
                  <a style="max-width: 150px; display:inline-block; vertical-align: middle; overflow: hidden; white-space: nowrap; text-overflow: ellipsis;" href="${readUrl}">${qnaList.content}</a>
                </td>
                <td class="text-center">${qnaList.writer}</td>
                <td class="text-center">${qnaList.editdate}</td>
                <td class="text-center">
                  <c:if test="${qnaList.board_confirm==1}">처리대기</c:if>
                  <c:if test="${qnaList.board_confirm==2}">처리완료</c:if>
                </td>
        ...
        <c:otherwise>
          <tr>
            <td colspan="5" class="text-center" style="line-height: 100px">조회된 서비스문의 목록이 없습니다.</td>
          </tr>
        </c:otherwise>
      </c:choose>
      </tbody>
				...
				<ul class="pagination">
					...
					<c:choose>
						<c:when test="${pageHelper.prevPage > 0}">
							...
							<c:url var="prevUrl" value="/admin/service/qna_list.do">
					...
					<c:forEach var="i" begin="${pageHelper.startPage}"
						...
						<c:url var="pageUrl" value="/admin/service/qna_list.do">
	                        <%-- <c:param name="keyword" value="${keyword}"></c:param> --%>
					...
					<c:choose>
						<c:when test="${pageHelper.nextPage > 0}">
							...
							<c:url var="nextUrl" value="/admin/service/qna_list.do">
							    <%-- <c:param name="keyword" value="${keyword}"></c:param> --%>
