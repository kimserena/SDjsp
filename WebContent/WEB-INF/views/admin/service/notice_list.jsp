...
<%--   <!-- 검색 폼 -->
  <div class="pull-left">
    <form method="get" action="${pageContext.request.contextPath}/admin/service/notice_list_list.do" style="width: 200px">
      ...
      <c:choose>
        <c:when test="${fn:length(noticeList) > 0}">
          <c:forEach var="noticeList" items="${noticeList}">
              <tr>
                <td class="text-center">${noticeList.idx}</td>
                <td class="text-center">
                  <c:url var="readUrl" value="/admin/service/notice_read.do">
                    <c:param name="noti_no" value="${noticeList.idx}"/>
                  </c:url>
                  <a href="${readUrl}">${noticeList.subject}</a>
                </td>
                <td class="text-center">${noticeList.writer}</td>
                <td class="text-center">${noticeList.editdate}</td>
				...
				<ul class="pagination">
					...
					<c:choose>
						<c:when test="${pageHelper.prevPage > 0}">
							...
							<c:url var="prevUrl" value="/admin/service/notice_list.do">
					...
					<c:forEach var="i" begin="${pageHelper.startPage}"
						...
						<c:url var="pageUrl" value="/admin/service/notice_list.do">
	                        <%-- <c:param name="keyword" value="${keyword}"></c:param> --%>
					...
					<c:choose>
						<c:when test="${pageHelper.nextPage > 0}">
							...
							<c:url var="nextUrl" value="/admin/service/notice_list.do">
							    <%-- <c:param name="keyword" value="${keyword}"></c:param> --%>
