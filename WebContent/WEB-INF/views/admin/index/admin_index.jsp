                        ...
                        <div class="navbar"> <a href="#home"><i class="fas fa-home"></i> <!--<span class="font-gray"><i class="xi-angle-right-thin"></i></span>--></a>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-content">
                                  ...
                                  <a href="${pageContext.request.contextPath}/admin/index/admin_index.do" class="dropbtn-color">고객관리</a> 
                            ...
                            <div class="dropdown">
                                ...
                                <div class="dropdown-content"> 
                                  <a href="${pageContext.request.contextPath}/admin/index/admin_index.do" class="dropbtn-color">거래처리스트</a> 
...
<div class="container">
  ...
  <!-- 검색 폼 -->
  <div class="pull-left">
    <form method="get" action="${pageContext.request.contextPath}/admin/index/admin_index.do" style="width: 200px">
	  ...
	  <tbody>
      <c:choose>
        <c:when test="${fn:length(newUserList) > 0}">
          <c:forEach var="newUserList" items="${newUserList}">
              <tr>
                <td class="text-center">${newUserList.useridx}</td>
                <td class="text-center">
                  <c:url var="readUrl" value="/admin/users/admin_new_user_info.do">
                    <c:param name="useridx" value="${newUserList.useridx}"/>
                  </c:url>
                  <a href="${readUrl}">${newUserList.userid}</a>
                </td>
                <td class="text-center">${newUserList.name}</td>
                <c:choose>
                  <c:when test="${newUserList.companyname eq null}">
                    <td class="text-center">*</td>
                  </c:when>
                  <c:otherwise>
                    <td class="text-center">${newUserList.companyname}</td>
                  </c:otherwise>
                </c:choose>
                <td class="text-center">${newUserList.ceoname}</td>
                <td class="text-center">${newUserList.busicondition}</td>
                <td class="text-center">${newUserList.cateofbusiness}</td>
                <td class="text-center">${newUserList.fax}</td>
                <td class="text-center">${newUserList.responsiblename}</td>
                <td class="text-center">${newUserList.responsibleemail}</td>
                <c:choose>
                  <c:when test="${newUserList.responsibletel eq null}">
                    <td class="text-center">*</td>
                  </c:when>
                  <c:otherwise>
                    <td class="text-center">${newUserList.responsibletel}</td>
                  </c:otherwise>
                </c:choose>
                
                <c:choose>
                  <c:when test="${newUserList.responsiblesellphon eq null}">
                    <td class="text-center">*</td>
                  </c:when>
                  <c:otherwise>
                    <td class="text-center">${newUserList.responsiblesellphon}</td>
                  </c:otherwise>
                </c:choose>
				...
				<ul class="pagination">
					...
					<c:choose>
						<c:when test="${pageHelper.prevPage > 0}">
							...
							<c:url var="prevUrl" value="/admin/index/admin_index.do">
					...
					<c:forEach var="i" begin="${pageHelper.startPage}"
						...
						<c:url var="pageUrl" value="/admin/index/admin_index.do">
	                        <c:param name="keyword" value="${keyword}"></c:param>
					...
					<c:choose>
						<c:when test="${pageHelper.nextPage > 0}">
							...
							<c:url var="nextUrl" value="/admin/index/admin_index.do">
							    <c:param name="keyword" value="${keyword}"></c:param>
