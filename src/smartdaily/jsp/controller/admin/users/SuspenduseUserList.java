...
/**
 * Servlet implementation class SuspenduseUserList
 */
@WebServlet("/admin/users/suspenduse_user_list.do")
public class SuspenduseUserList extends BaseController {

	/**
	 * 
	 */
	private static final long serialVersionUID = -7960940961421235891L;
	...
	public String doRun(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int totalCount = 0;
		List<Users> suspendUseUserList = null;
		
		try {
			// 전체 게시물 수 
			totalCount = usersService.selectSuspendUseUsersCount(users);
			// 나머지 페이지 번호 계산하기 
			...
			suspendUseUserList = usersService.selectAdminSuspendUseUserList(users);
		}catch(Exception e) {
		...
		request.setAttribute("suspendUseUserList", suspendUseUserList);
		request.setAttribute("pageHelper", pageHelper);
		
		return "admin/users/suspenduse_user_list";
	}
