...
/**
 * Servlet implementation class FarmViewAuthList
 */
@WebServlet("/admin/service/farm_view_auth_list.do")
public class FarmViewAuthList extends BaseController {

	/**
	 * 
	 */
	private static final long serialVersionUID = -2968777894047246198L;
	...
	public String doRun(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int totalCount = 0;
		List<Board> subscripList = null;
		
		
		/** (5) service를 통한 권한 요청 목록 조회  */
		try {
			//전체 게시물 수
			totalCount = boardService.selectBoardFarmViewAuthCount(board);
			//나머지 페이지 번호 계산하기
			...
			subscripList = boardService.selectBoardFarmViewAuthList(board);
		}catch(Exception e) {
		...
		/** (6) 조회 결과를 View에 전달 */
		request.setAttribute("subscripList", subscripList);
		request.setAttribute("pageHelper", pageHelper);
		
		
		return "admin/service/farm_view_auth_list";
	}
